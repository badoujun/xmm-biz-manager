...
public class RequestParameterWrapper extends HttpServletRequestWrapper {
     ...
     * 重要！spring根据request组装controller的入参对象时需调用
     */
    @Override
    ...
    /**
     * 重写getParameterNames，代表参数从当前类中的map获取
     * 重要！spring根据request组装controller的入参对象时需调用
     */
    @Override
    public Enumeration<String> getParameterNames() {
        Enumeration<String> enumeration = new Enumeration<String>() {
            List<String> paramsList = new ArrayList<>(params.keySet());
            int index = 0;

            @Override
            public boolean hasMoreElements() {
                return paramsList.size() > index;
            }

            @Override
            public String nextElement() {
                String paramName = paramsList.get(index);
                index += 1;
                return paramName;
            }
        };
